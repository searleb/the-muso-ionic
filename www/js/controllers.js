...
.controller('MusoDetailsCtrl', function($scope, $stateParams, musoService, $ionicModal, $ionicActionSheet, $ionicPopup, skillsService, $cordovaSocialSharing) {
	...
	$ionicModal.fromTemplateUrl('templates/modals/add-muso.html', {
	  ...
	  // sharing
	  $scope.share = function(muso){
	  	var skills = [];
	  	for (var i = 0; i < muso.instruments.length; i++) {
	  		var skill = muso.instruments[i]
	  		skills.push( " " + skill.charAt(0).toUpperCase() + skill.slice(1) );
	  	};
	  	var content = muso.firstName + " " + muso.lastName + " " + muso.phone + " " + muso.email + " " + skills;
	  	var title = "Contact: " + muso.firstName + " " + muso.lastName;
	  	$cordovaSocialSharing.share(content, title);
	  }
})
